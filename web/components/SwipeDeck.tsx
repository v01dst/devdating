...
function ProjectCard({ card, onSwipe }: { card: DiscoveryCard; onSwipe: (direction: SwipeDirection) => void }) {
  ...
  return (
    <motion.article
      ...
      className="glass-card absolute inset-x-0 bottom-24 mx-auto flex max-w-md flex-col rounded-[28px] p-6 shadow-glow"
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-wide text-accent-soft">{project.owner_login}</p>
          <h2 className="mt-1 text-2xl font-semibold leading-tight">{project.name}</h2>
        </div>
        <div className="rounded-2xl bg-black/40 px-3 py-2 text-center">
          <div className="text-lg font-bold text-like">{Math.round(card.compatibility_score)}</div>
          <div className="text-[10px] uppercase tracking-wide text-white/50">match</div>
        </div>
      </div>
      <p className="mt-4 line-clamp-4 min-h-20 text-sm text-white/70">{project.description ?? "No description provided."}</p>
      <div className="mt-5 flex flex-wrap gap-2">
        {project.languages.slice(0, 4).map((language) => (
          <span key={language} className="rounded-full border border-white/15 px-3 py-1 text-xs">{language}</span>
        ))}
        {project.topics.slice(0, 2).map((topic) => (
          <span key={topic} className="rounded-full bg-accent/25 px-3 py-1 text-xs">#{topic}</span>
        ))}
      </div>
      <div className="mt-6"><ActivityHeatmap score={project.activity_score} /></div>
      <ul className="mt-5 space-y-2 text-sm text-white/70">
        {card.reasons.slice(0, 2).map((reason) => (
          <li key={reason} className="flex gap-2"><span className="text-signal">✦</span>{reason}</li>
        ))}
      </ul>
    </motion.article>
  );
...
export function SwipeDeck() {
  ...
  if (cardsQuery.isLoading) {
    return <div className="mt-32 text-center text-white/60">Loading your matches…</div>;
  }

  if (!currentCard) {
    return (
      <div className="glass-card mt-24 mx-auto max-w-md rounded-3xl p-8 text-center">
        <h2 className="text-xl font-semibold">No more projects</h2>
        <p className="mt-3 text-white/60">Adjust preferences or refresh to discover a broader set.</p>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="absolute inset-x-0 bottom-8 flex justify-center gap-6">
        <button
          ...
          className="grid size-16 place-items-center rounded-full border border-pass/40 bg-pass/15 text-2xl transition hover:bg-pass/25"
          aria-label="Pass"
        >
          ✕
        </button>
        <button
          ...
          className="grid size-16 place-items-center rounded-full border border-like/40 bg-like/15 text-2xl transition hover:bg-like/25"
          aria-label="Like"
        >
          ♥
        </button>
      </div>
      {feedback && <output className="absolute inset-x-0 top-6 text-center text-sm text-white/75">{feedback}</output>}
    </>
  );
}
