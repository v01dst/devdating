...
export default async function IssuesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-6 py-12">
      ...
      <form action="/issues" className="card mt-8 grid gap-4 rounded-3xl p-5 md:grid-cols-[1fr_180px_180px_130px]">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{issues.length === 0 && <div className="card rounded-3xl p-6 text-zinc-600">No live issues indexed yet. Run <code>devdating sync-github</code>.</div>}
        {issues.map((issue: any) => (
          <a key={issue.issue_id} href={issue.url} target="_blank" rel="noreferrer" className="card group rounded-3xl p-6 transition hover:border-accent/40">
            <div className="flex items-center justify-between text-xs text-zinc-500"><span>{issue.project_name}</span><span className="text-emerald-600">{Math.round(issue.score)} fit</span></div>
